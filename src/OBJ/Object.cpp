...
namespace obj
{
  ...
  glt::Model MakeModel(Object const& i_object)
  {
    ...
    std::vector<glt::Vertex> vertexList;
    ...
    for (auto const& face : i_object.m_faceList)
    {
      for (auto vertexIndex = 0; vertexIndex < 3; ++vertexIndex)
      {
        ...
        auto searchResult = std::find(vertexList.begin(), vertexList.end(), vertex);
        if (searchResult != vertexList.end())
        {
          indexList.push_back(GLuint(searchResult-vertexList.begin()));
        }
        else
        {
          vertexList.push_back(vertex);
          indexList.push_back(GLuint(vertexList.size() - 1));
        }
      }
    }
